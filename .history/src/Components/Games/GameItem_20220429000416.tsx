...
export const GameItem: React.FC<PageProps> = (props: any, { game }) => {
  const { episodes, toggleFavAction, favorites, store } = useContext(Store)
    ...
    <div className={styles.card}>
      ...
      <div className={styles.card_body}>
        ...
        <div className={styles.card_footer}>
          ...
          <button
            type="button"
            onClick={() => toggleFavAction(state, dispatch, episodes)}
          >
            {favorites.find((fav: Game) => fav.id === episodes.id)
              ? 'Undo Favorite'
              : 'add Favorite'}
          </button>
        </div>
